...
void gl_poly_cxt_txr( pvr_poly_cxt_t *dst, pvr_list_t list,
	...
	dst->gen.shading = GL_SHADE_FUNC;
	
	dst->depth.comparison = PVR_DEPTHCMP_ALWAYS;
	dst->depth.write = DEPTH_ENABLE;
	
    dst->gen.culling = GL_CULL_FUNC;
...
void gl_poly_cxt_col(pvr_poly_cxt_t *dst, pvr_list_t list) {
	int alpha;
...
void gl_poly_compile(pvr_poly_hdr_t *dst, pvr_poly_cxt_t *src) {
	int	u, v;
	uint32	txr_base;
